...
export default function PaymentPage() {
    ...
    const checkPaymentStatus = () => {
        ...
        const carData = carUtils.getCarById(orders, Number(id));
        if (!carData) {
            setPaymentStatus('failed');
            return;
        }
        ...
        // Nếu đã thanh toán
        if (carData.paymentMethod === 'Đã thanh toán') {
            setPaymentStatus('success');
            return;
        }

        // Nếu chưa thanh toán → mô phỏng quá trình xử lý 5s
        setTimeout(() => {
            // Giả lập thành công
            carData.paymentMethod = 'Đã thanh toán';
            parsedData.totalOrder = (parsedData.totalOrder || 0) + 1;
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            {paymentStatus === 'success' && (
                                <div className="bg-green-50 border-2 border-green-200 rounded-2xl p-8 text-center">
                                    ...
                                    <button onClick={() => navigate("/user")} className="bg-green-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-green-700 transition-colors">
                                        Xem đơn hàng
                                    </button>
                                </div>
                            )}
